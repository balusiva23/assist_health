                  ...
                  <!--      <div class="col-md-2">
                       ...
                       <!--         <select class="form-control input-height search timing" name="timing" id="timing" required>
                        ...
                        <div class="form-group col-md-2">
							...
							<!--<input type="range" class="form-range" id="fees" name="fees" value="10" min="300" max="1500" step="50" onInput="updateTextInput(this.value)">
							<span id="feeval" style="vertical-align:top; line-height:5px"></span>-->
							<select class="form-control input-height search fees" name="fees" id="fees" required>
                                <option value="">Select...</option>
                                <?php
                        ...
                        <div class="form-group col-md-2">
                         ...
                         <div class="form-group col-md-2">
                            ...
                            <select class="form-control input-height search doctor_area areafieldsfilter" name="doctor_area" id="doctor_area" required>
                                <option value="">Select...</option>
                            </select>
                        </div> 
                        <!--<div class="form-group col-md-2">
                            <label class="control-label">Hospital Area <span class="required"> </span></label>
                            <select class="form-control input-height search hospital_area areafieldsfilter" name="hospital_area" id="hospital_area" required>
                                <option value="">Select...</option>
                            </select>
                        </div>-->
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
								...
								<select class="form-control input-height specialityselect" name="speciality" id="speciality"  required>
                                                        <option value="">Select...</option>
                        ...
                        <div class="form-group row">
                            <label class="control-label col-md-3">Specialised In <span class="required"></span>
                            </label>
                            <div class="col-md-8">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="text" name="hospital" id="hospital" data-required="1" placeholder="Enter Name " class="form-control input-height" required />
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <div class="input-group">
                                    ...
                                    <input name="number" id="number" maxlength="10" type="text" class="form-control input-height" placeholder="Contact Number" required />
                ...
                <div class="modal-body">
                                    ...
                                    <div class="card-body no-padding height-9">
                                        ...
                                        <ul class="list-group list-group-unbordered">
                                            ...
                                            <h4 class="text-center"><b>Work place 1 (Hospital Area)</b></h4>
                                            <li class="list-group-item">
                                            ...
                                            <h4 class="text-center"><b>Work place 2 (Clinic Area)</b></h4>
                                            <li class="list-group-item">
...
//         console.log(name)
//         $.ajax({
//             url:"<?php echo base_url(); ?>Navigator/fetch_data/"+page,
//             method:"POST",
//             dataType:"JSON",
		 ...
		 $.ajax({
			...
			success:function(resp){
				...
				if(areaselect!='') {
					...
					$('.'+selectClass+' option[value="'+areaselect+'"]').attr("selected","selected");
					$('.'+selectClass+' option[value="'+areaselect+'"]').prop("selected","selected");
...
$(document).ready(function () {
    ...
    $('.filtersection #name, .filtersection #speciality,.filtersection #hospital, .filtersection  #timing, .filtersection #fees ,.filtersection #hospital_area, .filtersection  #doctor_area, .filtersection #hospital_city').change(function () {
      // Remove selected options in other dropdowns
     //   $('#name, #speciality, #hospital, #timing, #fees').not(this).val('');
		if($(this).attr('id') == 'hospital_city') {
			loadArea($(this).attr('id'), $(this).val(), "areafieldsfilter");
	...
	$('#form_sample_1 #hospital_city, #form_sample_1 #clinic_city').change(function () {
    ...
    $('#resetFilters').click(function () {
        ...
        $('#name, #speciality, #hospital, #timing, #fees, #hospital_area, #doctor_area, #hospital_city').val('');

        // Trigger the change event for each dropdown to update the displayed text
        $('#name, #speciality, #hospital, #timing, #fees, #hospital_area, #doctor_area, #hospital_city').change();
    ...
    //new ----------------------
      function get_filters() {
        ...
        // Assuming your select elements have IDs: 'name', 'speciality', 'timing', 'fees'  #hospital_area, 
        $('#name, #speciality, #hospital, #timing, .filtersection #fees, #doctor_area, #hospital_city').each(function () {
...
// New--------------------------Filter---------------------------------
 ...
 // $(document).ready(function() {
       ...
       $(document).on('click','.tblEditBtn',function(){
            ...
            $.ajax({
                url: '<?php echo base_url("Navigator/getInternal_DoctorByID"); ?>?id=' + id,
                method: 'GET',
                data: { id: id },
                dataType: 'json',
                ...
                // Populate the modal with the data returned from the server
					...
					$('#staticBackdrop [name="speciality"] option[value="'+response.speciality+'"]').attr("selected","selected");
					$('#staticBackdrop [name="speciality"] option[value="'+response.speciality+'"]').prop("selected","selected");
					//$('#staticBackdrop [name="speciality"]').val(response.speciality);
					...
					$('#staticBackdrop [name="hospital_city"] option[value="'+response.hospital_city+'"]').attr("selected","selected");
					$('#staticBackdrop [name="hospital_city"] option[value="'+response.hospital_city+'"]').prop("selected","selected");
                    $('#staticBackdrop [name="clinic_area"]').val(response.clinic_area);
                    $('#staticBackdrop [name="clinic_city"]').val(response.clinic_city);
                    $('#staticBackdrop [name="clinic_city"] option[value="'+response.clinic_city+'"]').attr("selected","selected");
					$('#staticBackdrop [name="clinic_city"] option[value="'+response.clinic_city+'"]').prop("selected","selected");
					...
					loadArea("hospital_city", response.hospital_city, "hospital_city_area", response.hospital_area);
					loadArea("clinic_city", response.clinic_city,"clinic_city_area", response.clinic_area);
                     // Open the modal
                    $('#staticBackdrop').modal('show');
                },
    ...
    //});
        ...
        if($("#form_sample_1").valid() == false)
			return false;
         
         $.ajax({
        type:'post',
        url: '<?php echo base_url("Navigator/Update_internal_doctor");?>',
   ...
   $.confirm({
    ...
    $.ajax({
    type: 'post',
    url: '<?php echo base_url('Navigator/delete_internal_doctor') ?>',
    data: {id:id},
    success: function (response) {
   ...
   $(document).on('click','.tblViewBtn',function(){
            ...
            $.ajax({
                ...
                // Populate the modal with the data returned from the server
                    ...
                    $('#staticBackdrop_doc .speciality').text(response.specialityname);
                   ...
                   // $('#staticBackdrop_doc .clinic_timing').text(formatTime(response.clinic_timing));
                    ...
                    $('#staticBackdrop_doc .hospital_city').text(response.hospital_cityname);
                    $('#staticBackdrop_doc .clinic_area').text(response.clinic_areaname);
                    $('#staticBackdrop_doc .clinic_city').text(response.clinic_cityname);
                    $('#staticBackdrop_doc .hospital_area').text(response.hospital_areaname);
