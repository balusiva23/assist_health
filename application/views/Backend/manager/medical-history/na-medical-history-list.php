                                                    ...
                                                    <div class="card-body ">
                                                        ...
                                                        <table
                                                            ...
                                                            <tbody>
                                                               <?php 
                                                               $i = 1;
                                                                foreach ($members as $member) {if($member->isSubprofile){
                                                                 ...
                                                                 <tr class="odd gradeX">
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <a class="btn blue-bgcolor btn-outline btn-circle m-b-10" data-id="<?php echo $member->id; ?>" href="<?php echo base_url(); ?>Navigator/SubMedical_history?id=<?= 
                                                                        $member->id?>">
                                                                             <!-- <i class="fa fa-eye"></i> -->
                                                                            view
                                                                        </a>
...
<div class="modal fade" id="propertyImagemodel" tabindex="-1">
    ...
    if (selectedFiles.length > 0) {
       // if (selectedFiles.length <= maxAllowedFiles) {
            ...
            $.ajax({
                url: "<?php echo base_url('Navigator/add_bulk_medical_documents');?>",
                method: "POST",
                data: formData,
      ...
      $(document).ready(function () {
        ...
        if(id != ''  ){
        $.ajax({
          url: '<?php echo base_url("Navigator/deleteImage")?>',
          type:"POST",
          data: {id:id},          
          success: function(response) {
...
<script type="text/javascript">
 ...
 $(document).on('click','#update_member',function(){
        ...
        $.ajax({
        type:'post',
        url: '<?php echo base_url("Navigator/add_medical_history_document");?>',
...
$(document).on('click','.delete', function (e) {
   ...
   $.confirm({
    ...
    $.ajax({
    type: 'post',
    url: '<?php echo base_url('Navigator/delete_docu') ?>',
    data: {id:id},
    success: function (response) {
     var data=$.parseJSON(response);
     if(data.status == 'success'){
