                                                                ...
                                                                foreach ($members as $member) {  
                                                                 ...
                                                                 <tr class="odd gradeX">
                                                                    ...
                                                                    <td class="center">
                                                                        ...
                                                                        <a class="btn deepPink btn-outline btn-circle m-b-10"  data-id="<?php echo $member->id; ?>" href="<?php echo base_url(); ?>Doctor/Add_medical_history?id=<?php echo $member->id; ?>">
                                                                            Add/Update Medical History
                                                                        </a> 
                                                                        <a class="btn blue-bgcolor btn-outline btn-circle m-b-10"  data-id="<?php echo $member->id; ?>" href="<?php echo base_url(); ?>Doctor/View_medical_history?id=<?php echo $member->id; ?>">
                                                                    ...
                                                                    <td>
                                                                        ...
                                                                        <a class="btn blue-bgcolor btn-outline btn-circle m-b-10" data-id="<?php echo $member->id; ?>" href="<?php echo base_url(); ?>Doctor/SubMedical_history?id=<?= 
                                                                        $member->id?>">
                                                                             <!-- <i class="fa fa-eye"></i> -->
                                                                            view
                                                                        </a>
...
<div class="modal fade" id="propertyImagemodel" tabindex="-1">
    ...
    if (selectedFiles.length > 0) {
       // if (selectedFiles.length <= maxAllowedFiles) {
            ...
            $.ajax({
                url: "<?php echo base_url('Doctor/add_bulk_medical_documents');?>",
                method: "POST",
                data: formData,
       ...
       function loadproperty_images(){
        //$(document).ready(function () {
        $(document).on("click", '.tblEditBtn1',function (event) {
      ...
      $(document).ready(function () {
        ...
        if(id != ''  ){
        $.ajax({
          url: '<?php echo base_url("Doctor/deleteImage")?>',
          type:"POST",
          data: {id:id},          
          success: function(response) {
          ...
          $(document).ready(function () {
            ...
            $(document).on('click', ".tblEditBtn1", function (e) {
              e.preventDefault(e);
              var id = $(this).attr('data-id');
...
<script type="text/javascript">
 ...
 $(document).on('click','#update_member',function(){
        ...
        $.ajax({
        type:'post',
        url: '<?php echo base_url("Doctor/update_member_data");?>',
   ...
   $.confirm({
    ...
    $(document).on('click','#update_member',function(){
        ...
        $.ajax({
        type:'post',
        url: '<?php echo base_url("Doctor/add_medical_history_document");?>',
        data: new FormData($("#member_form_add")[0]),
        contentType: false,
        processData: false, 
        success:function(resp){
   ...
   $.confirm({
    ...
    $.ajax({
    type: 'post',
    url: '<?php echo base_url('Doctor/delete_docu') ?>',
    data: {id:id},
    success: function (response) {
     var data=$.parseJSON(response);
     if(data.status == 'success'){
