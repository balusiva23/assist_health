...
public function Get_medical_Documents(){
   ...
   // $getdata = $this->Common_model->Get_medical_Documents($id);
	...
	echo' <tr>
		...
		<td style="text-align:center"><a title="Download" href="'.base_url().'assets/uploads/medical_history/'.$value->file.'" class="  text-primary" data-id="'.$value->id.'" download>   <i class=" fa fa-download"></i></a></td>
	</tr>';
   ...
   public function PreMembers()
   {
	   ...
	   $data['members'] = $this->Manager_model->Get_prescription_data($id);
   
	   $data['allmembers'] = $this->Manager_model->Get_prescription_data($id);
	   $this->load->view('Backend/members/pre-members',$data);
   } 
    ///-----------------prescription pdf--------------------
