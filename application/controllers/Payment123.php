...
class Payment extends CI_Controller {
    ...
    public function __construct() {
       ...
       //$this->phonePe = new PhonePe('PGTESTPAYUAT85', '88186875-58c6-4313-a000-1e640b3db251', 1); // Use the correct parameters
        $this->phonePe = new PhonePe('M22GP7C1IXZX4', 'd90972b6-380b-4d97-ab35-945e4e4fd978', 1); // Use the correct parameters
    }
...
public function initiate_payment() {
   ...
   $callback_url = base_url('payment/callback?name=' . urlencode($name) . '&email=' . urlencode($email) . '&password=' . urlencode($password) . '&number=' . urlencode($number) . '&code=' . urlencode($code). '&plan=' . urlencode($plan));
    ...
    try {
        ...
        $response = $this->phonePe->PaymentCall($transaction_id, $email, $amount, $callback_url, $callback_url, $number, 'PROD');// UAT
...
public function callback() {
    ...
    // $name = json_decode($this->input->get('name'));
    // $email = json_decode($this->input->get('email'));
    // $password = json_decode($this->input->get('password'));
    // $number = json_decode($this->input->get('number'));
    // $code = json_decode($this->input->get('code'));
        // Retrieve user data from query parameters
    ...
    if (isset($response['transactionId'], $response['code'], $response['checksum'])) {
        ...
        // if ($this->validate_checksum($response)) {
            if ($responseCode === 'PAYMENT_SUCCESS') {
                ...
                $member_id = $this->savemember($name, $email, $password, $number, $code,$plan);
                // echo $member_id; 
                // echo "T!"; 
                if ($member_id) {
                    ...
                    $transaction_data = array(
                     ...
                     $result = $this->User_model->save_transaction($transaction_data);
                    $t_amt = $amount / 100;
                    if ($result) {
                        //mail
                        $this->send_mail($name,$email,$number,$transaction_data['transaction_id'],$plan_value,$t_amt );
...
public function savemember($name, $email, $password, $number, $code,$plan) {
    ...
    //         return $this->db->insert_id();  // Return the member ID for further processing
    //     } else {
...
public function send_mail($name,$email,$number,$transaction_id,$plan_value,$t_amt) {
