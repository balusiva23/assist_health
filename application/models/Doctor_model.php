...
class Doctor_model extends CI_Model {
	...
	public function getAll_Areadetails()
    {
        		
		$this->db->select('areadetails.*, cities.cityname');
        $this->db->from('areadetails'); //new change
         
        $this->db->join('cities', 'cities.id = areadetails.cityid');
		$this->db->order_by('areaname', 'asc');
		 $query = $this->db->get();
		
        return $query->result();
    }
	public function getAll_Speciality()
    {
        $this->db->where(array('isActive'=>1));
        $query = $this->db->get('speciality');
        return $query->result();
    }
	public function get_SpecialityById($id) {
	...
	public function get_allareasByCityId($id) {

		$this->db->where('cityid', $id);
		$query = $this->db->get('areadetails');
		return $query->result();
    } 
	public function get_AreadetailsById($id) {
   ...
   // Subprofile
    ...
    public function Update_Speciality($data, $id)
    {
        $this->db->where('id', $id);
	...
	public function save_Areadetails($data)
    {
        return $this->db->insert('areadetails', $data);
    }
	
    public function Update_Areadetails($data, $id)
    {
        $this->db->where('id', $id);
		return $this->db->update('areadetails', $data);
    }
    ...
    public function getAll_InternalDoctors()
    {
        ...
        $query = $this->db->get('internal_doctors');
        return $query->result();*/
		 $this->db->select('internal_doctors.*, speciality.name as specialityname');
        $this->db->from('internal_doctors'); //new change
         $this->db->where(array('internal_doctors.isActive'=>1));
        $this->db->join('speciality', 'internal_doctors.speciality=speciality.id', 'left');
        $query = $this->db->get();
       return $result = $query->result();
    }
...
function make_query($filters)
{
    ...
    foreach ($filters as $filter) {
        ...
        if ($value !== '') {
			...
			if ($field === 'doctor_area') {
				$this->db->group_start()
						 ->where('clinic_area', $value)
						 ->or_where('hospital_area', $value)
						 ->group_end();
			} else {
    ...
    function fetch_data($limit, $start, $filters)
    {
        ...
        if ($data->num_rows() > 0) {
		  ...
		  if(is_numeric($doctor->speciality)){
			$specialityArr = $this->db->get_where('speciality', ['id' => $doctor->speciality])->row();
			$specialityname = $specialityArr ->name;
		}else
                            ...
                            <div class="card-body no-padding" style="height: 320px;">
                                <div class="doctor-profile">
                                    ...
                                    <div class="profile-usertitle">
                                        ...
                                        <div class="name-center">' . $specialityname . '</div>
                                    </div>
                                    <p>' . $doctor->hospital . '<br /></p>
                                    <div>
